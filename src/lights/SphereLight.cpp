...
SphereLight::SphereLight(const Vector3D &position, const Colour &intensity, double radius) : LightSource(position, intensity), radius(radius) {}

SphereLight::SphereLight(JsonObject json) : SphereLight(Vector3D(json["position"].asArray()), Colour(json["intensity"].asArray()), json["radius"].asDouble()) {}
